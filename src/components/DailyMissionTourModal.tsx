...
import { Mail, Sparkles, ArrowLeft, Check, Heart, X } from "lucide-react";
...
export default function DailyMissionTourModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-[#070b14]/90 backdrop-blur-xl animate-fade-in overflow-y-auto">
      ...
      <div className="relative w-full max-w-lg rounded-3xl bg-gradient-to-b from-slate-900/95 via-[#0b101d]/95 to-slate-950/98 border border-amber-500/25 p-6 sm:p-8 shadow-[0_12px_60px_rgba(0,0,0,0.7),0_0_30px_rgba(245,158,11,0.15)] flex flex-col items-center text-center">
        {onClose && (
          <button
            type="button"
            onClick={onClose}
            className="absolute top-5 left-5 w-8 h-8 rounded-full bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700 text-slate-400 hover:text-slate-100 flex items-center justify-center transition-colors"
            title="بستن"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="relative w-14 h-14 rounded-2xl bg-gradient-to-b from-rose-500/20 to-amber-500/10 border border-rose-500/30 flex items-center justify-center mb-4 shadow-[0_0_20px_rgba(244,63,94,0.25)]">
          <MartyrTulipIcon className="w-9 h-9" />
        </div>

        {/* ── Phase 1: Warm Welcome Message ── */}
        {phase === "welcome" && (
          <div className="flex flex-col items-center animate-fade-in">
            <span className="text-xs font-bold text-amber-400/90 tracking-widest uppercase mb-1">
            ...
            <div className="w-12 h-0.5 bg-gradient-to-l from-transparent via-amber-500/60 to-transparent mb-4" />

            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed text-center mb-6 max-w-md">
            ...
            <div className="p-4 rounded-2xl bg-amber-500/[0.06] border border-amber-500/20 text-xs text-amber-200/90 flex items-center gap-3 mb-7 text-right">
        ...
        {phase === "envelope" && (
          <div className="w-full flex flex-col items-center animate-fade-in">
            ...
            <button
              ...
              className={`w-full sm:w-auto px-9 py-3.5 rounded-2xl font-bold text-sm flex items-center justify-center gap-2.5 transition-all duration-300 shadow-[0_0_25px_rgba(225,29,72,0.35)] ${
                isOpeningEnvelope
                  ? "bg-slate-800 text-slate-400 cursor-not-allowed scale-95"
                  : "bg-gradient-to-l from-rose-600 via-rose-500 to-rose-600 text-white hover:brightness-110 active:scale-[0.98]"
              }`}
            >
        ...
        {phase === "revealed" && (
          <div className="w-full flex flex-col items-center animate-scale-up">
            <div className="w-full p-6 sm:p-7 rounded-2xl bg-gradient-to-b from-[#1a141e]/90 to-[#0e0c14]/90 border border-amber-500/35 shadow-[0_8px_30px_rgba(0,0,0,0.5),0_0_20px_rgba(245,158,11,0.15)] mb-5 text-center relative overflow-hidden">
              ...
              <h3 className="text-xl sm:text-2xl font-black text-transparent bg-clip-text bg-gradient-to-l from-amber-200 via-amber-300 to-rose-300 my-2">
                {mission.martyr.name}
              </h3>

              <span className="inline-block text-[11px] text-amber-400/80 bg-amber-500/10 px-3 py-1 rounded-full border border-amber-500/20 mb-4">
                {mission.martyr.title}
              </span>

              <div className="w-16 h-px mx-auto bg-gradient-to-l from-transparent via-amber-500/50 to-transparent my-3" />

              <div className="my-3 py-3 px-4 rounded-xl bg-slate-950/70 border border-slate-800">
            ...
            <button
              ...
              className="w-full sm:w-auto px-9 py-3.5 rounded-2xl bg-gradient-to-l from-emerald-600 via-emerald-500 to-emerald-600 text-white font-bold text-sm shadow-[0_0_24px_rgba(16,185,129,0.35)] hover:shadow-[0_0_32px_rgba(16,185,129,0.5)] active:scale-[0.98] transition-all duration-300 flex items-center justify-center gap-2"
            >
